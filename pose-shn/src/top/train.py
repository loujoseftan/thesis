...
if __name__ == "__main__":
    ...
    net = Hourglass(num_classes=16, num_channels=256, num_stacks=args.num_stack, inres=(256, 256), outres=(64, 64))

    if args.resume:
        net.resume_train(batch_size=args.batch_size, model_json=args.resume_model_json,
                          model_weights=args.resume_model,
                          init_epoch=args.init_epoch, epochs=args.epochs)
    else:
        net.create_model(show_model=False)
        net.train(epochs=args.epochs, model_path=args.model_path, batch_size=args.batch_size)

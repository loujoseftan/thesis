...
folder = 'C:\Users\LJ\Desktop\dataset\run\run-frames'; %file directory of the folder of folders of segmented videos per frame
list = dir(folder); %list of videos
n=length(list)-1; %number of videos
% for i=1:n  %uncomment for annotating all videos (1000) at once
for i = 83:102 %uncomment to choose videos to annotate
...
%     txt1=jsonencode(table(Z1(:,:,6),Z1(:,:,5),Z1(:,:,4),Z1(:,:,3),Z1(:,:,2),Z1(:,:,1),Z1(:,:,7),Z1(:,:,8),Z1(:,:,9),Z1(:,:,10),Z1(:,:,16),Z1(:,:,15),Z1(:,:,14),Z1(:,:,13),Z1(:,:,12),Z1(:,:,11)));
    ...
    fid = fopen(['C:\Users\LJ\Desktop\dataset\run\run\' filenamefin '.json'], 'w'); %input directory of folder with raw videos
